...
import { reAssignStocktakingArea, createStocktaking, updateStocktaking } from '../../services/StocktakingService';
import { extractErrorMessage } from '../../utils/Validation';
import dayjs from 'dayjs';

const AssignSingleAreaModal = ({
    isOpen,
    onClose,
    onSuccess,
    stocktakingSheetId,
    stocktaking = null,
    formData = null,
    areaIdToReassign // areaId (int) của khu vực cần phân công lại
}) => {
    ...
    }, [isOpen, stocktakingAreasStr, areaIdToReassign]);
    ...
    const fetchAreaAndEmployees = async () => {
        ...
        try {
            ...
            if (stocktakingSheetId) {
                ...
                const targetArea = areasArray.find(a => (a.areaId || a.id) === areaIdToReassign);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (window.showToast) {
                if (formData) {
                    if (stocktakingSheetId) {
                        window.showToast('Cập nhật và phân công lại thành công!', 'success');
                    } else {
...
export default AssignSingleAreaModal;
