...
import { getGoodsIssueReport } from "../../services/DashboardService"
...
/**
 * Export Sales Orders Report Component
 * 
 * @param {Object} props
 * @param {string} props.searchQuery - Search query string
 * @param {string} props.retailerFilter - Selected retailer ID
 ...
 */
export default function ExportSalesOrdersReport({
    searchQuery = "",
    retailerFilter = "",
    dateRange = {
    ...
    const handleExport = async () => {
        try {
            // Hiển thị loading
            if (window.showToast) {
                window.showToast("Đang xuất báo cáo đơn bán hàng...", "info")
            }
            ...
            const excelData = filteredData.map((item, index) => {
                return {
                    ...
                    "Ngày xuất": item.issueDate ? formatDate(item.issueDate) : "-"
                }
            })
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "Báo cáo đơn bán hàng")
            ...
            const fileName = `Bao_cao_don_ban_hang_${dayjs().format('DDMMYYYY_HHmmss')}.xlsx`
